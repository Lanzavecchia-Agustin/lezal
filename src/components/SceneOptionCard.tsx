...
import { AlertTriangle, ChevronRight, Skull, Zap } from 'lucide-react'
...
export function SceneOptionCard({
  ...
  return (
    <div className="border border-purple-700 p-4 rounded-lg bg-gray-900 shadow-lg shadow-purple-900/20 text-gray-200">
      <p className="font-semibold text-lg mb-2 text-purple-300">{option.text}</p>
      <div className="space-y-2 text-sm">
        ...
        {option.roll && (
          <div className="bg-gray-800 p-3 rounded-md">
            <p className="mb-1">
              Dificultad: <strong className="text-yellow-400">{option.roll.difficulty}</strong>, 
              Habilidad: <strong className="text-cyan-400">{getSubskillName(option.roll.skillUsed)}</strong> 
            ...
            {localRollResult[option.id] !== undefined && (
              <p className="mt-2">
                Resultado: <strong className="text-yellow-400">{localRollResult[option.id]}</strong> vs {option.roll.difficulty} 
                → <span className={localRollResult[option.id]! >= option.roll.difficulty ? "text-green-400" : "text-red-400"}>
          ...
          {option.successEffects && (
            <div className="flex-1 bg-green-900 p-2 rounded-md text-green-200">
              ...
              {option.successEffects.life !== undefined && (
                <p>Vida: {option.successEffects.life >= 0 ? "+" : ""}{option.successEffects.life}</p>
              )}
              {option.successEffects.stress !== undefined && (
                <p>Estrés: {option.successEffects.stress >= 0 ? "+" : ""}{option.successEffects.stress}</p>
              )}
            </div>
          )}
          {option.failureEffects && (
            <div className="flex-1 bg-red-900 p-2 rounded-md text-red-200">
              ...
              {option.failureEffects.life !== undefined && (
                <p>Vida: {option.failureEffects.life >= 0 ? "+" : ""}{option.failureEffects.life}</p>
              )}
              {option.failureEffects.stress !== undefined && (
                <p>Estrés: {option.failureEffects.stress >= 0 ? "+" : ""}{option.failureEffects.stress}</p>
              )}
            </div>
          )}
        </div>
